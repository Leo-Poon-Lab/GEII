...
selected_gene_type = st.sidebar.selectbox('Gene Type', ["Entrez ID", "Gene Symbol"])
if selected_gene_type == "Entrez ID":
    selected_gene_id = st.sidebar.text_input('ID', "1")
else:
    selected_gene_id = st.sidebar.text_input('ID', "BRCA1")

selected_exp_type = st.sidebar.selectbox('Experiment Type', ["Infection", "Stimulation"])
